...
class WindowClickerApi:
    ...
    def get_screenshot(self):
        ...
        # drop the alpha channel, or cv.matchTemplate() will
        # throw an error like:
    ...
    # translate a pixel position on a screenshot image to a pixel position
    # on the screen.
    # pos = (x, y)
    ...
    def print_screen_position(self):
        return (
            f"window position is:\n"
            f"X position: {self.offset_x}\n"
            f"Y position: {self.offset_y}\n"
            f"Width:      {self.w}\n"
            f"Height:     {self.h}"
        )
    ...
    def take_SS(self, save_path):
        region = (self.offset_x, self.offset_y, self.w, self.h)
